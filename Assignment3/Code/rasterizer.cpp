...
// 实现与作业二类似的插值算法，实现法向量、颜色、纹理颜色的插值
void rst::rasterizer::rasterize_triangle(const Triangle &t, const std::array<Eigen::Vector3f, 3> &view_pos) {
    ...
    // 1. 找到四边形，然后遍历四边形，叉积判断像素是否在三角形内
    // 2. 对于每一个在三角形内的像素，计算其深度、法线、纹理插值

    // t.toVector4() 的类型：std::array<Vector4f, 3>
    auto triData = t.toVector4();

    // 寻找四边形包围盒
    float rectangle_minX, rectangle_minY, rectangle_maxX, rectangle_maxY;
    rectangle_minX = rectangle_minY = INT_MAX;
    rectangle_maxX = rectangle_maxY = INT_MIN;

    // 对于每一个顶点
    for (auto vertex : triData) {
        rectangle_minX = std::min(vertex.x(), rectangle_minX);
        rectangle_maxX = std::max(vertex.x(), rectangle_maxX);
        rectangle_minY = std::min(vertex.y(), rectangle_minY);
        rectangle_maxY = std::max(vertex.y(), rectangle_maxY);
    }

    auto v = t.v;
    for (auto x = floor(rectangle_minX); x != ceil(rectangle_maxX); x++) {
        for (auto y = floor(rectangle_minY); y != ceil(rectangle_maxY); y++) {
